<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:99:"C:\xampp\htdocs\phpsystem\public/../application/front\view\paperSubject\paperSubject_frontlist.html";i:1550587161;s:77:"C:\xampp\htdocs\phpsystem\public/../application/front\view\common\header.html";i:1550642346;s:77:"C:\xampp\htdocs\phpsystem\public/../application/front\view\common\footer.html";i:1538061378;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<script>
		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<?php echo url('PaperSubject/frontAdd'); ?>" style="display:none;">添加试卷题目</a></li>
				</ul>
			  	<!-- Tab panes -->
				    			...
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					...
				    					<?php
				    	            		...
				    	            		/*遍历记录*/
				    					if(is_array($paperSubjectRs) || $paperSubjectRs instanceof \think\Collection): $i = 0; $__LIST__ = $paperSubjectRs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$paperSubject): $mod = ($i % 2 );++$i;?>
 										<tr>
 											<td><?php echo $currentIndex++; ?></td>
 											<td><?php echo $paperSubject['psId']; ?></td>
 											<td><?php echo $paperSubject['paperObjF']['paperName']; ?></td>
 											<td><?php echo $paperSubject['subjectObjF']['title']; ?></td>
 											<td><?php echo $paperSubject['addTime']; ?></td>
 											<td>
 												<a href="<?php echo url('PaperSubject/frontshow',array('psId'=>$paperSubject['psId'])); ?>"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="paperSubjectEdit('<?php echo $paperSubject['psId']; ?>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="paperSubjectDelete('<?php echo $paperSubject['psId']; ?>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="paperSubjectQueryForm" id="paperSubjectQueryForm" action="<?php echo url('PaperSubject/frontlist'); ?>" class="mar_t15" method="post">
            <div class="form-group">
...
function paperSubjectEdit(psId) {
	$.ajax({
		url :  "<?php echo url('PaperSubject/update'); ?>?psId=" + psId ,
		type : "get",
		dataType: "json",
		success : function (paperSubject, response, status) {
			if (paperSubject) {
				...
				$.ajax({
					url: "<?php echo url('PaperSubject/listAll'); ?>",
					type: "get",
					dataType: "json",
...
function paperSubjectDelete(psId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url: "<?php echo url('PaperSubject/deletes'); ?>",
			data : {
				psIds : psId,
			},
			dataType: "json",
			success : function (obj) {
				if (obj.success) {
					...
					//location.href="<?php echo url('PaperSubject/frontlist'); ?>";
				}
				else 
					alert(obj.message);
			},
...
function ajaxPaperSubjectModify() {
	$.ajax({
		url :  "<?php echo url('PaperSubject/update'); ?>",
		type : "post",
		dataType: "json",
