...
<script>
		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	<li><a href="__PUBLIC__/index.php">首页</a></li>
			    	<li role="presentation" class="active"><a href="#videoListPanel" aria-controls="videoListPanel" role="tab" data-toggle="tab">学习视频列表</a></li>
			    	<li role="presentation" ><a href="<?php echo url('Video/frontAdd'); ?>" style="display:none;">添加学习视频</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>视频id</td><td>视频标题</td><td>视频文件</td><td>发布老师</td><td>发布时间</td><td>操作</td></tr>
				    					<?php
				    						/*计算起始序号*/
				    	            		$startIndex = ($currentPage-1) * $rows;
				    	            		$currentIndex = $startIndex+1;
				    	            		/*遍历记录*/
				    					if(is_array($videoRs) || $videoRs instanceof \think\Collection): $i = 0; $__LIST__ = $videoRs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$video): $mod = ($i % 2 );++$i;?>
 										<tr>
 											<td><?php echo $currentIndex++; ?></td>
 											<td><?php echo $video['videoId']; ?></td>
 											<td><?php echo $video['title']; ?></td>
 											<td>
 											  <?php if($video['videoFile'] != ''): ?>
 											  <a href='__PUBLIC__/<?php echo $video['videoFile']; ?>' target='_blank'><?php echo $video['videoFile']; ?></a>
 											  <?php else: ?>
 											  暂无文件
 											  <?php endif; ?>
 											</td>
 											<td><?php echo $video['teacherObjF']['teacherName']; ?></td>
 											<td><?php echo $video['videoTime']; ?></td>
 											<td>
 												<a href="<?php echo url('Video/frontshow',array('videoId'=>$video['videoId'])); ?>"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="videoEdit('<?php echo $video['videoId']; ?>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="videoDelete('<?php echo $video['videoId']; ?>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="videoQueryForm" id="videoQueryForm" action="<?php echo url('Video/frontlist'); ?>" class="mar_t15" method="post">
			<div class="form-group">
				<label for="title">视频标题:</label>
				<input type="text" id="title" name="title" value="<?php echo $title; ?>" class="form-control" placeholder="请输入视频标题">
			</div>
            <div class="form-group">
			...
			<div class="form-group">
				<label for="videoTime">发布时间:</label>
				<input type="text" id="videoTime" name="videoTime" class="form-control"  placeholder="请选择发布时间" value="<?php echo $videoTime; ?>" onclick="SelectDate(this,'yyyy-MM-dd')" />
			</div>
...
<div id="videoEditDialog" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog" style="width:900px;" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;学习视频信息编辑</h4>
      </div>
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
		  	 ...
		  	 <div class="col-md-9">
			    <input type="text" id="video_title_edit" name="video_title" class="form-control" placeholder="请输入视频标题">
			 </div>
		  </div>
		  <div class="form-group">
		  	 ...
		  	 <div class="col-md-9">
			    ...
			    <input type="hidden" id="video_videoFile_edit" name="video_videoFile"/>
		  ...
		  <div class="form-group">
		  	 <label for="video_teacherObj_teacherNo_edit" class="col-md-3 text-right">发布老师:</label>
		  	 <div class="col-md-9">
			    <select id="video_teacherObj_teacherNo_edit" name="video_teacherObj_teacherNo" class="form-control">
			    </select>
		  	 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="video_videoTime_edit" class="col-md-3 text-right">发布时间:</label>
		  	 <div class="col-md-9">
                <div class="input-group date video_videoTime_edit col-md-12" data-link-field="video_videoTime_edit">
                    <input class="form-control" id="video_videoTime_edit" name="video_videoTime" size="16" type="text" value="" placeholder="请选择发布时间" readonly>
...
function videoEdit(videoId) {
	$.ajax({
		url :  "<?php echo url('Video/update'); ?>?videoId=" + videoId ,
		type : "get",
		dataType: "json",
		success : function (video, response, status) {
			if (video) {
				...
				$("#video_videoFile_edit").val(video.videoFile);
				$("#video_videoFileA").text(video.videoFile);
				$("#video_videoFileA").attr("href", "__PUBLIC__/" +　video.videoFile);
				video_videoDesc_edit.setContent(video.videoDesc, false);
				$.ajax({
					url: "<?php echo url('Video/listAll'); ?>",
					type: "get",
					dataType: "json",
					success: function(teachers,response,status) { 
		        		...
		        		$("#video_teacherObj_teacherNo_edit").val(video.teacherObj);
					}
				});
...
function videoDelete(videoId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url: "<?php echo url('Video/deletes'); ?>",
			data : {
				videoIds : videoId,
			},
			dataType: "json",
			success : function (obj) {
				if (obj.success) {
					...
					//location.href="<?php echo url('Video/frontlist'); ?>";
				}
				else 
					alert(obj.message);
			},
...
/*ajax方式提交学习视频信息表单给服务器端修改*/
function ajaxVideoModify() {
	$.ajax({
		url :  "<?php echo url('Video/update'); ?>",
		type : "post",
		dataType: "json",
